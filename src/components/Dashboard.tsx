...
import { DollarSign, Package, Calendar, TrendingUp, AlertTriangle, CheckCircle } from 'lucide-react';
...
const upcomingEvents = [
  { name: 'TechCrunch Disrupt', deadline: '2024-01-15', match: 95 },
  { name: 'Y Combinator Demo Day', deadline: '2024-01-20', match: 88 },
  { name: 'Startup Grind', deadline: '2024-01-25', match: 72 }
];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Events & tenders matched</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Breakdown of your current month expenses</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                ...
                <p className="text-sm text-muted-foreground">$960/year renewal</p>
              </div>
            ...
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Printer paper, pens</p>
              </div>
            ...
            {upcomingEvents.map((event, index) => (
              <div key={index} className="p-3 border rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <h4>{event.name}</h4>
                  <Badge variant="outline">{event.match}% match</Badge>
                </div>
                <p className="text-sm text-muted-foreground mb-2">Deadline: {event.deadline}</p>
                <Progress value={event.match} className="h-2" />
              </div>
            ))}
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 border rounded-lg hover:bg-muted/50 cursor-pointer transition-colors">
              ...
              <p className="text-sm text-muted-foreground">Upload and extract invoice data</p>
            </div>
            <div className="p-4 border rounded-lg hover:bg-muted/50 cursor-pointer transition-colors">
              <TrendingUp className="h-6 w-6 text-blue-500 mb-2" />
              <h4>Run Financial Forecast</h4>
              <p className="text-sm text-muted-foreground">Predict next quarter expenses</p>
            </div>
            <div className="p-4 border rounded-lg hover:bg-muted/50 cursor-pointer transition-colors">
              <Package className="h-6 w-6 text-purple-500 mb-2" />
              <h4>Check Tool Alternatives</h4>
              <p className="text-sm text-muted-foreground">Find cheaper SaaS options</p>
            </div>
          </div>
        </CardContent>
      </Card>
